...
import { Download, Shield, Lock, Key, AlertCircle, CheckCircle2, X, FileText, Eye, EyeOff, Mail } from 'lucide-react';
...
export const ExportPdfModal: React.FC<ExportPdfModalProps> = ({
  ...
  const [providerConfirmEmail, setProviderConfirmEmail] = useState('');
  ...
  const targetEmail = (userProfile?.email || '').trim().toLowerCase();
  ...
  const handleExport = async (e: React.FormEvent) => {
    ...
    // 2. Identity Verification & Export Authorization:
    // When 2FA is set, TOTP code authorizes export (password is not required).
    // When 2FA is NOT set and user is Email auth: account password is required.
    // When 2FA is NOT set and user is Provider auth (Google/OAuth): require email confirmation.
    if (has2FA) {
      ...
      if (cleanCode.length !== 6) {
        setError('Authenticator 2FA code must be exactly 6 digits.');
        return;
      }
      ...
      const cleanAccountPassword = accountPassword.trim();
      if (!cleanAccountPassword) {
      ...
      if (accountsRaw) {
        try {
          ...
          if (acc?.passwordHash) {
            let hash = 0;
            for (let i = 0; i < cleanAccountPassword.length; i++) {
              const char = cleanAccountPassword.charCodeAt(i);
              hash = (hash << 5) - hash + char;
              hash |= 0;
            }
            const enteredHash = 'phash_' + Math.abs(hash).toString(36) + '_' + cleanAccountPassword.length;
          ...
          console.warn('Local account password verification notice:', e);
        }
      }
    } else {
      // Provider Login (Google/OAuth) without 2FA:
      // Require confirmation of the registered account email to ensure explicit authorization
      const cleanConfirmEmail = providerConfirmEmail.trim().toLowerCase();
      if (!cleanConfirmEmail) {
        setError(`Please enter your account email (${userProfile?.email || 'registered email'}) to authorize this export.`);
        return;
      }

      if (targetEmail && cleanConfirmEmail !== targetEmail) {
        setError(`Confirmation email does not match your active profile (${userProfile?.email}).`);
        return;
      }
    }
      ...
      try {
        ...
        if (userProfile?.uid) {
          await recordExportDownload(userProfile.uid, {
            ...
            securityMethod: has2FA
              ? 'Password Encrypted + 2FA TOTP'
              : isEmailAuth
              ? 'Password Encrypted + Account Password'
              : 'Password Encrypted + Provider Email Verified'
          });
        }
  ...
  const isSubmitDisabled =
    isExporting ||
    !filePassword.trim() ||
    (has2FA && totpCode.replace(/\s+/g, '').length !== 6) ||
    (!has2FA && isEmailAuth && !accountPassword.trim()) ||
    (!has2FA && !isEmailAuth && targetEmail && providerConfirmEmail.trim().toLowerCase() !== targetEmail);
        ...
        >
          ...
          <div className="overflow-y-auto flex-1 pr-1 space-y-4">
            ...
            <form onSubmit={handleExport} className="space-y-4">
              ...
              {/* 2. Identity Verification & Export Authorization */}
              {has2FA ? (
                /* Mode A: 2FA Active - Code authorizes export without requiring account password */
                <div
                  className="p-3.5 rounded-xl border space-y-2"
                  style={{
                ...
                >
                  <div className="flex items-center justify-between">
                    <label
                      ...
                      className="block text-xs font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-400"
                    >
                      2. Authenticator 2FA Verification
                    </label>
                    <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1">
                      <Shield className="w-3 h-3" />
                      2FA Active &bull; No Password Required
                    </span>
                  </div>
                  <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                    Enter the current 6-digit code from your authenticator app to authorize downloading your reflections.
                  </p>
                  <div className="relative">
                    <input
                      ...
                      disabled={isExporting}
                      className="w-full text-center text-xl font-mono font-bold tracking-[0.3em] py-2.5 px-3.5 rounded-xl border transition-all focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
                      style={{
                        backgroundColor: 'var(--bg-card-elevated)',
                        borderColor: 'var(--border-color)',
                        color: 'var(--text-primary)'
                    ...
                    />
                    <Key className="absolute right-3.5 top-3 w-4 h-4 opacity-40 pointer-events-none text-emerald-500" />
                  </div>
                </div>
              ) : isEmailAuth ? (
                /* Mode B: Email Auth without 2FA - Fallback to Account Password */
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                      Enter your account login password to authorize exporting your private reflections.
                    </p>
                    <div className="relative">
                      <input
                      ...
                      <button
                        ...
                        className="absolute right-3 top-2.5 p-1 rounded hover:opacity-80 transition cursor-pointer"
                        title={showAccountPassword ? 'Mask password' : 'Show password'}
                        aria-label={showAccountPassword ? 'Mask password' : 'Show password'}
                        style={{ color: 'var(--text-secondary)' }}
                      >
                  ...
                  {/* 2FA Recommendation for Email Auth */}
                  <div
                  ...
                  >
                    ...
                    <p className="text-[11px] leading-relaxed">
                      Enable 2FA in Settings to authorize reflection exports with your authenticator app instead of your account password.
                    </p>
                    {onOpenTwoFactorSetup && (
                      <button
                        type="button"
                ...
                /* Mode C: Provider Auth (Google/OAuth) without 2FA - Explicit Account Email Confirmation */
                <div className="space-y-3">
                  <div
                    className="p-3.5 rounded-xl border space-y-2"
                    style={{
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <label
                        htmlFor="export-provider-email-input"
                        className="block text-xs font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400"
                      >
                        2. Identity Confirmation ({userProfile?.authProvider ? userProfile.authProvider.toUpperCase() : 'Google'} Account)
                      </label>
                    ...
                    <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                      Since you signed in via {userProfile?.authProvider || 'Google'} without 2FA, please enter your registered email address (<strong className="font-mono">{userProfile?.email}</strong>) to confirm your identity and authorize the export.
                    </p>
                    <div className="relative">
                      <input
                        id="export-provider-email-input"
                        type="email"
                        placeholder={`Enter ${userProfile?.email || 'your account email'}`}
                        value={providerConfirmEmail}
                        onChange={(e) => {
                          setProviderConfirmEmail(e.target.value);
                          if (error) setError(null);
                        }}
                        disabled={isExporting}
                        className="w-full text-xs py-2.5 px-3.5 pr-10 rounded-xl border transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                        style={{
                          backgroundColor: 'var(--bg-card-elevated)',
                          borderColor: 'var(--border-color)',
                          color: 'var(--text-primary)'
                      ...
                      />
                      <Mail className="absolute right-3.5 top-3 w-4 h-4 opacity-40 pointer-events-none text-blue-500" />
                    </div>
                  </div>

                  {/* 2FA Recommendation for Provider Auth */}
                  <div
                    className="p-3 rounded-xl border text-xs space-y-1.5"
                    style={{
                      backgroundColor: 'var(--bg-card-elevated)',
                  ...
                  >
                    ...
                    <p className="text-[11px] leading-relaxed">
                      Enable 2FA to protect your reflections with instant 6-digit authenticator verification instead of typing your email.
                    </p>
                    {onOpenTwoFactorSetup && (
                      <button
                        type="button"
